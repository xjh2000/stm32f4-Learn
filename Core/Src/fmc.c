...
//在指定地�?(WriteAddr+Bank5_SDRAM_ADDR)�?�?,连续写入 n 个字�?.
//pBuffer:字节指针
//WriteAddr:要写入的地址
//n:要写入的字节�?
...
//在指定地�?((WriteAddr+Bank5_SDRAM_ADDR))�?�?,连续读出 n 个字�?.
//pBuffer:字节指针
//ReadAddr:要读出的起始地址
//n:要写入的字节�?
...
//设置自刷新次�?
    temp = (uint32_t) SDRAM_MODEREG_BURST_LENGTH_1 |//设置突发长度:1
           ...
           SDRAM_MODEREG_CAS_LATENCY_3 |//设置 CAS �?:3(可以�? 2/3)
           SDRAM_MODEREG_OPERATING_MODE_STANDARD | //标准模式
           SDRAM_MODEREG_WRITEBURST_MODE_SINGLE; //单点访问
    SDRAM_Send_Cmd(0, FMC_SDRAM_CMD_LOAD_MODE, 1, temp); //发�?�命�?
