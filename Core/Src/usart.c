...
extern osThreadId dataUpdateTaskHandle;
extern osThreadId dataPushTaskHandle;

uint8_t uart1data[201];
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart) {
    ...
    if (huart == &huart2) {
        HAL_UART_Receive_IT(uart2ReceiveData.handle, (uint8_t *) &uart2ReceiveData.itBuff, 1);
        BaseType_t xHigherPriorityTaskWoken;
        xHigherPriorityTaskWoken = pdFALSE;
        if (!uart2ReceiveData.receiveSuccess) {
            if (uart2ReceiveData.dataLen + 1 >= uart2ReceiveData.dataMaxLen) {
                uart2ReceiveData.dataLen = 0;
            } else {
                uart2ReceiveData.data[uart2ReceiveData.dataLen] = uart2ReceiveData.itBuff;
                uart2ReceiveData.dataLen++;
                if (uart2ReceiveData.dataLen == uart2ReceiveData.data[1]) {
                    uart2ReceiveData.receiveSuccess = 1;
                    // 通知推�?�处�?
