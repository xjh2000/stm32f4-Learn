...
uint32_t HAL_GetTimeMs(void) {
    return osKernelSysTick() * 1000 / configTICK_RATE_HZ;
}
...
void hal_thread_create(volatile void *threadHandle, uint16_t stackSize, int Priority, void (*fn)(void *), void *arg) {
    osThreadDef(parseTask, (os_pthread) fn, (osPriority) Priority, 0, stackSize);
    threadHandle = osThreadCreate(osThread(parseTask), arg);
    (void) threadHandle; //Eliminate warning
}

void hal_thread_destroy(void *threadHandle) {
