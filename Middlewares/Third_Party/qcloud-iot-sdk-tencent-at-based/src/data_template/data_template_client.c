...
int IOT_Template_Register_Property(void *handle, DeviceProperty *pProperty, OnPropRegCallback callback) {
    ...
    if (template_common_check_property_existence(ptemplate, pProperty)) IOT_FUNC_EXIT_RC(QCLOUD_ERR_PROPERTY_EXIST);
...
static void
_reply_ack_cb(void *pClient, Method method, ReplyAck replyAck, const char *pReceivedJsonDocument, void *pUserdata) {
...
#ifdef QUOTES_TRANSFER_NEED
    rc_of_snprintf = HAL_Snprintf(jsonBuffer + strlen(jsonBuffer) - pos - 1, remain_size,
                                  ""T_",\\\"device_label\\\":{");
#else
...
#ifdef QUOTES_TRANSFER_NEED
    rc_of_snprintf = HAL_Snprintf(jsonBuffer, remain_size, "{\\\"code\\\":%d"T_", \\\"clientToken\\\":\\\"%s\\\""T_",",
                                  replyPara->code, get_control_clientToken());
#else
...
#ifdef QUOTES_TRANSFER_NEED
        rc_of_snprintf = HAL_Snprintf(jsonBuffer + strlen(jsonBuffer), remain_size, "\\\"status\\\":\\\"%s\\\"}",
                                      replyPara->status_msg);
#else
